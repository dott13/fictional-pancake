...
public class TextArchitect
{
    ...
    public float speed { get { return baseSpeed * speedMultiplied; } set { speedMultiplied = value; } }
    private const float baseSpeed = 2;
    private float speedMultiplied = 1;
    ...
    private IEnumerator Build_Fade()
    {
        ...
        while (true) 
        {
            float fadeSpeede = ((hurryUp ? charactersPerCycle * 5 : charactersPerCycle) * speed) * 4;
            for (int i = minRange; i < maxRange; i++)
            {
                ...
                alphas[i] = Mathf.MoveTowards(alphas[i], 255, fadeSpeede);
